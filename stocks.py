...
def ch4_stock_update(annual_emissions: list, parameters: c.ch4parameters):
    ...
    for annual in annual_emissions:
       ...
       # GRAPHING BOUNDARY ALIGNMENT: 
       # Because the arrays are seeded with the Year 0 baseline, running the full emission 
       # loop creates a timeline of length (time_horizon + 1). To match the dashboard's 
       # strict display limit, we slice the arrays to return exactly the requested window.
    ...
    target_length = len(annual_emissions)
    return current_stock, np.array(oxidation_loss[:target_length]), np.array(ch4_stock_update[:target_length]), np.array( ch4_ppb_update[:target_length])
