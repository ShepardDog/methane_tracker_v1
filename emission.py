...
#for v1 version of the app we will assume that wetland emissions remain constant over time, but for v2 wetland emission will be considered as a function of temprature and time.
    ...
    #v2: Will integrate temperature_data loops seamlessly here.
        ...
        return [base_emission] * time_horizon
...
def project_humane_phaseout(time_horizon: int):
    ...
    return sum(emissions), emissions      
        
def calc_sector_emissions( sector: str, growth_rate: float, time_horizon: int):
    
    if time_horizon == 0 :
        return []
    
    if sector == "animal_ag" and growth_rate < 0: 
...
#Calculating fossi fuel growth rates 
...
if __name__ == "__main__":
    ...
    animal_ag_emissions = calc_sector_emissions("animal_ag", animal_ag_growth_rate, time_horizon)
    print(f"Animal Agriculture Emissions: {animal_ag_emissions}")
    
    #fossil_fuel_emissions = calc_fossil_fuel_emissions("coal", 0.0, time_horizon)
