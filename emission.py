...
def calc_growth_rate( sector: str,emission_adjusement: float, time_horizon: int):
    ...
    val.validate_time_horizon(time_horizon)
    val.validate_emission_adjustement_value(emission_adjusement)
    if time_horizon == 0:
        return 0
...
#for v0.5 version of the app we will assume that wetland emissions remain constant over time, but for v2 wetland emission will be considered as a function of temprature and time.
    val.validate_time_horizon(time=time_horizon)
    if time_horizon == 0 :
        return np.zeros(0, dtype=float)
    
    else:
    #Calculates natural wetland emissions. 
    #v0.5: Constant time list. 
    #v1: Will integrate temperature_data loops seamlessly here.
   
        base_emission = c.ch4_emission.get("wetlands", 180)
...
def project_humane_phaseout(time_horizon: int):
    ...
    emissions_humane = base_emission * (1 - (t/max_time)** power)
    #emissions_humane = []
...
def calc_sector_emissions( sector: str, emission_adjustment: float, time_horizon: int):
    ...
    val.validate_time_horizon(time=time_horizon)
    val.validate_emission_adjustement_value(emission_adjustment)
    if time_horizon == 0 :
        return np.zeros(0, dtype=float) 
    ...
    if sector == "animal_ag" and emission_adjustment < 0: 
        ...
        emissions =E0 * np.exp(growth_rate * np.arange(time_horizon))
        ...
        growth_rate = calc_growth_rate(sector, emission_adjustment, time_horizon)
        #emissions = [E0 * math.exp(growth_rate * t) for t in range(time_horizon)]
        emissions = E0 * np.exp(growth_rate * np.arange(time_horizon, dtype=float))
        return np.round(emissions, 2)     
...
def calc_fossil_fuel_emissions (fuel:str, emission_adjustement: float, time_horizon: int):
    val.validate_time_horizon(time=time_horizon)
    val.validate_emission_adjustement_value(emission_adjustement)
    if time_horizon == 0 :
        return np.zeros(0, dtype=float)
#Calculate BAU senario 
    if emission_adjustement == 0:
        ...
        #emissions = [E_base]
        base_emissions = np.array([E_base])
       ...
       # phase1_emissions = [E_base * math.exp(rate_1 * t) for t in range(1, years_p1)]
        ...
        phase1_emissions = E_base * np.exp(rate_1 * np.arange(1, years_p1))

        #emissions.extend(phase1_emissions)
        emissions = np.concatenate((base_emissions, phase1_emissions))
        ...
        growth_rate = math.log(1+rate_2)/p2
        ...
        phase2_emissions = E_phase2_initial * np.exp(growth_rate * np.arange(1, years_p2 + 1))
        emissions = np.concatenate((emissions, phase2_emissions))
        #phase2_emissions = [E_phase2_initial * math.exp(growth_rate * t) for t in range(1, years_p2 + 1)]
        ...
        growth_rate = math.log(1+rate_3)/p3
        ...
        phase3_emissions = E_phase3_initial* np.exp(growth_rate * np.arange(1, years_p3 + 1))
        #phase3_emissions = [E_phase3_initial * math.exp(growth_rate * t) for t in range(1, years_p3 + 1)]
        #emissions.extend(phase3_emissions)
        
        # Returns exactly the time_horizon length with clean 2-decimal rounding
        emissions = np.concatenate((emissions, phase3_emissions))  
        return np.round(np.array(emissions[:time_horizon]), 2)
...
#     
        ...
        emissions = E0 * np.exp(rate * np.arange(time_horizon))
...
if __name__ == "__main__":
   ...
   # wetland_emissions = calc_wetland_emissions(time_horizon)
    #print("Paddy:", np.array(wetland_emissions).shape)
    #print(f"Wetland Emissions: {wetland_emissions}")
    
    animal_ag_emissions = calc_sector_emissions("animal_ag", -1, time_horizon)
    #print("animal af:", np.array(animal_ag_emissions).shape)
    ...
    #print("Paddy:", np.array(paddy_emissions).shape)
    print(f"Paddy Emissions: {paddy_emissions}")
    
    fossil_fuel_emissions = calc_fossil_fuel_emissions("gas", 0.0, time_horizon)
    #print("fossil:", np.array(fossil_fuel_emissions).shape)
    #print (f"Fossil Fuel Emissions: {fossil_fuel_emissions}")
   ...
   # print("landfill_emissions:", np.array(landfill_emissions).shape)
